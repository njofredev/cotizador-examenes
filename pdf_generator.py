...
def generar_cotizacion_pdf(folio_cot, folio_om, timestamp_emision, nombre_p, doc_id, fecha_nac, prevision, df_sel, t_f, t_c, t_pg, t_pp, pack_nombre=None, incluir_om=False):
    ...
    def renderizar_contenido(pdf_obj, fase_conteo=False):
        ...
        pdf_obj.ln(5)
        pdf_obj.cell(0, 10, "COTIZACIÓN DIGITAL DE EXÁMENES", ln=True, align='C')
        pdf_obj.ln(5)
        
        pdf_obj.set_font("Arial", 'B', 10)
        pdf_obj.cell(0, 6, "DATOS DEL PACIENTE:", ln=1)
        pdf_obj.set_font("Arial", '', 10)
        pdf_obj.cell(0, 6, f"Nombre: {nombre_p}", ln=1)
        pdf_obj.cell(0, 6, f"Documento: {doc_id}", ln=1)
        pdf_obj.cell(0, 6, f"Edad: {calcular_edad(fecha_nac)} años", ln=1)
        pdf_obj.cell(0, 6, f"Previsión: {prevision}", ln=1)
        pdf_obj.ln(5)
        ...
        for i, h in enumerate(headers):
            pdf_obj.cell(w[i], 10, h, 1, 0, 'C', True)
        ...
        for _, r in df_sel.iterrows():
            ...
            pdf_obj.cell(w[0], 8, str(r['Código']), 1, 0, 'C')
            nombre_pdf = str(r['Nombre'])
            if len(nombre_pdf) > 45: nombre_pdf = nombre_pdf[:42] + "..."
            pdf_obj.cell(w[1], 8, f" {nombre_pdf}", 1)
            pdf_obj.cell(w[2], 8, str(int(r['Cant'])), 1, 0, 'C')
            ...
            pdf_obj.cell(w[3], 8, f"${(v1*r['Cant']):,.0f}", 1, 0, 'R')
            pdf_obj.cell(w[4], 8, f"${(v2*r['Cant']):,.0f}", 1, 1, 'R')
        
        if pdf_obj.get_y() > 210: pdf_obj.add_page()

        pdf_obj.set_font("Arial", 'B', 9)
        pdf_obj.set_fill_color(240, 240, 240)
        pdf_obj.cell(w[0]+w[1]+w[2], 10, "Total estimado a pagar", 1, 0, 'R', True)
        res1, res2 = (t_f, t_c) if prevision == "Fonasa" else (t_pg, t_pp)
        pdf_obj.cell(w[3], 10, f"${res1:,.0f}", 1, 0, 'R', True)
        pdf_obj.cell(w[4], 10, f"${res2:,.0f}", 1, 1, 'R', True)

        pdf_obj.ln(2)
        pdf_obj.set_font("Arial", 'I', 8)
        pdf_obj.cell(w[0]+w[1]+w[2], 6, f"Total exámenes cotizados: {int(df_sel['Cant'].sum())}", 0, 1, 'R')
        
        pdf_obj.ln(5)
        pdf_obj.set_font("Arial", 'B', 8)
        pdf_obj.cell(0, 5, "SUCURSAL LABORATORIO TOMA DE MUESTRAS:", ln=True)
        pdf_obj.set_font("Arial", '', 8)
        ...
        if incluir_om:
            ...
            pdf_obj.set_font("Arial", 'B', 14)
            pdf_obj.ln(5)
            pdf_obj.cell(0, 10, "ORDEN MÉDICA DE EXÁMENES", ln=True, align='C')
            pdf_obj.ln(5)
            ...
            for i, h in enumerate(headers_om):
                pdf_obj.cell(w_om[i], 10, h, 1, 0, 'C', True)
            ...
            for _, r in df_sel.iterrows():
                ...
                pdf_obj.cell(w_om[0], 10, str(r['Código']), 1, 0, 'C')
                pdf_obj.cell(w_om[1], 10, f" {str(r['Nombre'])[:70]}", 1)
                pdf_obj.cell(w_om[2], 10, str(int(r['Cant'])), 1, 1, 'C')

            if os.path.exists("firma_prado.png"):
                if pdf_obj.get_y() > 210: pdf_obj.add_page()
                pdf_obj.ln(15)
                curr_y = pdf_obj.get_y()
                pdf_obj.image("firma_prado.png", x=77.5, y=curr_y, w=55)
                pdf_obj.set_y(curr_y + 25)
                pdf_obj.set_font("Arial", 'B', 9)
